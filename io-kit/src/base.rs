...
use crate::registry::keys::kIORegistryEntryOptionsTypeNone;
...
impl IOService {
  pub fn get_matching_service(matching: CFDictionary) -> Option<IOService> {
    unsafe {
      let result = IOServiceGetMatchingService(kIOMasterPortDefault, matching.as_CFTypeRef() as _);
  ...
  pub fn get_matching_services(matching: CFDictionary) -> Result<Vec<Self>, i32> {
    unsafe {
      let mut io_iterator_t: MaybeUninit<io_iterator_t> = MaybeUninit::uninit();
      ...
      loop {
        let result = IOIteratorNext(io_iterator_t.assume_init());
...
pub trait IORegistryEntry {
  fn create_cf_properties(&self) -> Option<CFDictionary>;
}

impl IORegistryEntry for IOService {
  fn create_cf_properties(&self) -> Option<CFDictionary> {
    unsafe {
      let mut raw_dict_pointer: MaybeUninit<*mut __CFDictionary> = MaybeUninit::uninit();
      let result = IORegistryEntryCreateCFProperties(
        self.0,
        raw_dict_pointer.as_mut_ptr() as *mut _,
        kCFAllocatorDefault,
        kIORegistryEntryOptionsTypeNone,
      );

      if result != KERN_SUCCESS {
        None
      } else {
        let dict = CFDictionary::wrap_under_create_rule(raw_dict_pointer.assume_init());
        Some(dict)
      }
    }
  }
